        ...
        if user_input:
            try:
                ...
                if response.status_code == 200:
                    ...
                    # 4. 결과 시각화
                    if label == "POSITIVE":
                        st.success(f"😊 긍정적인 문장입니다! (확신도: {score:.2f})")
                    else:
                        st.error(f"😞 부정적인 문장입니다... (확신도: {score:.2f})")
                ...
                st.error(f"연결 실패! FastAPI 서버(8000번)가 켜져 있나요? \n 에러: {e}")
...
with tab2:
    ...
    if uploaded_file is not None:
        ...
        if st.button("이미지 분석하기", key="img_btn"):
            try:
                ...
                st.error(f"연결 실패! FastAPI 서버(8001번)가 켜져 있나요? \n 에러: {e}")